...
public struct NewMoveHelper
{
	...
	/// </example>
	public NewMoveHelper( Vector3 position, Vector3 velocity, params string[] solidTags ) : this()
	{
		Velocity = velocity;
		Position = position;
		GroundBounce = 0.0f;
	...
	/// var move = new MoveHelper( Position, Velocity )
	/// </example>
	public NewMoveHelper( Vector3 position, Vector3 velocity ) : this( position, velocity, "solid", "playerclip", "passbullets", "player" )
	{
	...
	public float TryMove( float timestep )
	{
		...
		for ( int bump = 0; bump < moveplanes.Max; bump++ )
		{
			...
			if ( !moveplanes.TryAdd( pm.Normal, ref Velocity, HitFloor ? GroundBounce : WallBounce ) )
				break;
		}
