...
public class Cockroach : NPC
{
	...
	{
		base.Touch( other );
		if ( other is HLPlayer && Game.IsServer )
		{
			Kill();
		}
	}
			...
			case "ROACH_EAT":
				{
					...
					if ( Game.Random.Int( 0, 3 ) == 1 )
					{
						...
						if ( COND_SEE_FEAR )
						{
							...
							Eat( 30 + (Game.Random.Int( 0, 14 )) );// roach will ignore food for 30 to 45 seconds
							PickNewDest( "ROACH_SCARED_BY_ENT" );
							SetActivity( "ACT_WALK" );
						}
						else if ( Game.Random.Int( 0, 149 ) == 1 )
						{
